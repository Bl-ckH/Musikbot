...
const github = 'https://www.nicklas.live';
const bot_version = require('../../package.json').version;

const bot_name = typeof (process.env.BOT_NAME) === 'undefined' ? 'Musik' : (ENV.BOT_NAME);
...
module.exports = {
    Embed_play: function (status, music_title, music_url, music_length, music_thumbnail, settings) {
        const Embed_play = new Discord.EmbedBuilder()
            ...
            .addFields({ name: status, value: `**Dauer**: \`${music_length}\` | ${settings}`, inline: true })
            .setTimestamp()
        return Embed_play;
    },
    ...
    Embed_queue: function (status, nowplay, queueMsg, loopStatus) {
        const Embed_queue = new Discord.EmbedBuilder()
            ...
            .setTimestamp()
            .setFooter({ text: `Schleife: ${loopStatus}` });
        return Embed_queue;
    },
